...
// even without .m3u8/.mpd extension (e.g., YouTube's /api/manifest/hls_playlist/...)
}  // namespace
...
std::optional<MediaKind> ClassifyMediaResponse(
  ...
  // When the MIME type tells us nothing, try to derive one from the URL extension.
  // Fallback to checking HLS/DASH MIME types directly (without URL pattern matching).
